...
export function createWebhookRouter({ db, config }) {
  ...
  router.post('/events', async (req, res, next) => {
    ...
    try {
      ...
      const recipient = (await client.query(`select r.id,r.email,c.tenant_id from campaign_recipients r join campaigns c on c.id=r.campaign_id join mailboxes m on m.id=c.mailbox_id where r.provider_message_id=$1 and m.provider=$2 for update`, [providerMessageId, provider])).rows[0];
